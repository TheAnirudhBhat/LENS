...
// `linkToDetail` turns the card into a control that opens the Portfolio score
// tab — the full breakdown of what moves the composite. Overview passes it;
// the score tab renders the same card plain so it doesn't link to itself.
// `hideHeader` drops the card's own title/subtitle — on the Portfolio score
// tab the PageHero already says it, so repeating it inside the box is noise.
export default function PortfolioScoreCard({
  ...
  hideHeader?: boolean;
  /** The Portfolio score tab shows the task queue directly below this card,
   *  and the levers are those same actions — suppress them there. */
  hideLevers?: boolean;
} = {}) {
  ...
  const levers = hideLevers ? [] : (score.levers ?? []).slice(0, 3);
  ...
  const card = (
    <Card>
      ...
      {/* Hero: big composite numeral + grade chip + band-tinted gauge.
          Restores its own top padding when the header above is suppressed. */}
      <div className={`px-6 pb-5 ${hideHeader ? "pt-6" : ""}`}>
      ...
      {/* Rule breaches — the rulebook's checks that are not part of the score */}
      {!hideLevers && (score.warnings?.length ?? 0) > 0 && (
        <section className="px-6 py-5" style={{ borderTop: "1px solid var(--border)" }}>
          <div className="text-[11px] font-medium text-tertiary mb-3">
            Rule breaches{score.rulesVersion ? ` · rulebook ${score.rulesVersion}` : ""}
          </div>
          <ul className="space-y-2">
            {(score.warnings ?? []).map((w) => (
              <li key={w} className="text-[12px] text-secondary leading-snug">
                {w}
              </li>
            ))}
          </ul>
  ...
  const openDetail = () =>
    window.dispatchEvent(new CustomEvent("dashboard:open-score"));

  return (
    <div
      role="button"
      tabIndex={0}
      aria-label={`Portfolio score ${composite} of 100 — open the full breakdown`}
      onClick={openDetail}
      onKeyDown={(e) => {
      ...
      className="block w-full text-left cursor-pointer rounded-lg transition-opacity
                 hover:opacity-90 focus-visible:outline-none focus-visible:ring-2
                 focus-visible:ring-offset-2 focus-visible:ring-[var(--brand)]"
    >
      {card}
    </div>
  );
