...
export default function AllocationTab({
  ...
  const summary = useMemo(() => {
    ...
    const byTarget = new Map<Role, number>();
    for (const r of data.roles) {
      byRole.set(r.role, r.weightPct);
      byTarget.set(r.role, r.targetPct);
    }
    // Asset-class targets are DERIVED from the Book's role targets (single
    // source of truth — they follow profile.json), never hardcoded. Equity =
    // compounders + growth + cyclicals + defensives; debt = debt-equiv; gold =
    // hedges. Sums to the SAA 85/10/5.
    const equityRoles: Role[] = ["compounders", "growth", "cyclicals", "defensives"];
    const equityPct = equityRoles.reduce((s, r) => s + (byRole.get(r) ?? 0), 0);
    const equityTarget = equityRoles.reduce((s, r) => s + (byTarget.get(r) ?? 0), 0);
    return {
      total: data.total,
      equityPct,
      equityTarget,
      debtPct: byRole.get("debt-equiv") ?? 0,
      debtTarget: byTarget.get("debt-equiv") ?? 0,
      hedgesPct: byRole.get("hedges") ?? 0,
      hedgesTarget: byTarget.get("hedges") ?? 0,
    };
  }, [data]);
