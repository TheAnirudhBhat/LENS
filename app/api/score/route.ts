...
// Classification lives in lib/book (bucketOf — role-first, meta/ticker
// fallback) so the SAA buckets here can never drift from the Overview strip
// or /api/sync totals.
...
export async function GET() {
  try {
    ...
    for (const h of snap?.holdings ?? []) {
      ...
      if (value <= 0) continue;
      const kind = bucketOf({ ticker: h.ticker, role: h.role });
      if (kind === "metals") buckets.metals += value;
      else if (kind === "bonds") buckets.bonds += value;
      else buckets.inEquity += value;
      positions.push({
        ticker: h.ticker,
        name: getMeta(h.ticker).name,
        market: kind === "bonds" ? "BONDS" : "IN",
        role: h.role,
    ...
    // ── Stable-bonds-platform SDI/NCD — ACTIVE rows only. Transferred rows
    // live in the Kite snapshot and are classified there via bucketOf; adding
    // them here double-counted the Kite-demat bonds (once as snapshot value,
    // once at bonds.json invested value). Matured are history. ──────────────
