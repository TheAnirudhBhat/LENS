...
async function readCrossSiloValue(): Promise<{
  usINR: number;
  mfINR: number;
  ...
  try {
    // Stable-Money ACTIVE bonds live OUTSIDE the Kite snapshot (the 4
    // transferred SDIs are already inside totalValue). Include them so the
    // persisted totalPortfolioValue matches the Book's net worth — it was
    // omitting the ~₹80.8K active Stable book (D2).
    const raw = await readFile(BONDS_FILE, "utf8");
    const parsed = JSON.parse(raw) as {
      totals?: { activeValueINR?: number; activeInvestedINR?: number };
    };
    stableBondsINR =
      parsed?.totals?.activeValueINR ?? parsed?.totals?.activeInvestedINR ?? 0;
  } catch {
    // no bonds silo — contributes 0
  }
  return { usINR, mfINR, stableBondsINR };
}
...
async function* runSync(): AsyncGenerator<Event> {
  ...
  for (const h of snapshot.holdings) {
    ...
    // Canonical classification via lib/book — equityValue keeps its existing
    // semantics (equity + metals; bonds and cash excluded).
    const bucket = bucketOf({ ticker: h.ticker, role: h.role as string });
    if (bucket === "bonds") {
      bondsValue += h.value;
    } else if (bucket !== "cash") {
      equityValue += h.value;
  ...
  const { usINR, mfINR, stableBondsINR } = await readCrossSiloValue();
  const totalPortfolioValue = totalValue + usINR + mfINR + stableBondsINR;
  ...
  if (deltas.length > 0) {
    ...
    for (const d of deltas) {
      // Canonical classification via lib/book (role-first, meta fallback).
      const snapHolding = snapshotByTicker.get(d.ticker);
      const bucket = bucketOf({ ticker: d.ticker, role: snapHolding?.role as string });
      const asset = bucket === "cash" ? "in-equity" : bucket;
      newEntries.unshift({
        id: `d${nextN++}`,
        date: todayStr,
  ...
  const histFile: { history: Array<{ date: string; totalValue: number; inValue?: number; usValue?: number; mfValue?: number; bondsValue?: number; cashInjection?: number; withdrawals?: number; nifty?: number | null; note?: string }> } = JSON.parse(histRaw);
  const todayStr = today.toISOString().slice(0, 10);
  ...
  // history.totalValue is FULL-PORTFOLIO basis (IN + US + MF), matching the
  // cron (scripts/auto-refresh.py) and chat /portfolio-check writers. Writing
  // IN-only here previously let whichever writer ran last flip the day's basis,
  // producing a fake ~64% cliff in any trend/return chart. Split fields make
  // each row self-describing so /api/history never has to guess the basis.
  const existing = histFile.history.find((h) => h.date === todayStr);
  if (existing) {
    existing.totalValue = Math.round(totalPortfolioValue);
    existing.inValue = Math.round(totalValue);
    existing.usValue = Math.round(usINR);
    existing.mfValue = Math.round(mfINR);
    existing.bondsValue = Math.round(bondsValue);
    ...
    histFile.history.push({
      ...
      inValue: Math.round(totalValue),
      usValue: Math.round(usINR),
      mfValue: Math.round(mfINR),
