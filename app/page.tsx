...
import { splitSnapshotByAsset, bondBookINR, netWorthINR } from "@/lib/book";

// Minimal client-side shape of the /api/valuation Book — only the fields the
// Overview reads. Declared locally (not imported from lib/valuation) so the
// client bundle never pulls in that module's node-only deps (fs, kite, etc.).
type OverviewBook = {
  totals: { netWorth: number; pnlAbs: number; invested: number };
  byAssetClass: Record<
    "in-equity" | "us-equity" | "mf" | "bonds" | "metals" | "cash",
    { value: number }
  >;
};
import { ageState, overdueDays } from "@/lib/taskAge";
import { US_RESEARCH, SCORE_BANDS } from "@/lib/policy";
import {
  computeRisk,
  concentrationLabel,
...
type Snapshot = {
  asOf: string;
  totalValue: number;
  // Full-book total (IN + US + MF), written by /api/sync. Used for drawdown
  // (vs the full-book peakValue) so the basis matches.
...
const NAV_GROUPS: NavGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      // Tab id stays "tasks" so existing deep links and events keep working;
      // the page is the score breakdown plus the actions that move it.
      { id: "tasks", label: "Portfolio score" },
...
export default function Dashboard() {
  ...
  // `dashboard:open-score` comes from the Overview score card — same landing
  // spot, since that tab now leads with the score breakdown.
  useEffect(() => {
    const onOpenTask = () => setTab("tasks");
    window.addEventListener("dashboard:open-task", onOpenTask);
    window.addEventListener("dashboard:open-score", onOpenTask);
    return () => {
      window.removeEventListener("dashboard:open-task", onOpenTask);
      window.removeEventListener("dashboard:open-score", onOpenTask);
    };
  }, [setTab]);
...
const REFRESH_SOURCE_LABELS: Record<string, { label: string; refreshHow: string }> = {
  ...
  mutualFunds: {
    label: "Mutual funds",
    refreshHow: "Run /portfolio-check — refreshed via the INDmoney MCP into project_mutual_funds.md",
  },
  usStocks: {
    label: "US equity (INDmoney)",
    refreshHow: "Run /portfolio-check — INDmoney MCP → us_stocks.json",
  },
};
...
const overviewCache: {
  ...
  stableBonds: { value: number; count: number } | null;
...
function OverviewTab({ goToTab }: { goToTab: (id: TabId) => void }) {
  ...
  // Canonical Book — the single source of truth for headline totals. Overview
  // prefers this so its net worth / allocation match the Allocation tab exactly
  // (both project /api/valuation). Falls back to the per-silo inline math below
  // if the Book hasn't loaded yet.
  const [book, setBook] = useState<OverviewBook | null>(overviewCache.book);
  const [bondsFetchedAt, setBondsFetchedAt] = useState<string | null>(overviewCache.bondsFetchedAt);
  const [stableBonds, setStableBonds] = useState<{ value: number; count: number } | null>(
    overviewCache.stableBonds
  );
  ...
  useEffect(() => {
    ...
    const loadUs = () =>
      fetch("/api/usstocks")
        ...
        .catch(() => {});
    // Canonical Book — one fetch that drives the headline totals + allocation,
    // identical to what the Allocation tab reads. /api/valuation returns the
    // Book object directly (no { data } wrapper).
    const loadBook = () =>
      fetch("/api/valuation")
        .then((r) => r.json())
        .then((b) => {
          if (b && !b.error && b.totals) {
            overviewCache.book = b;
            setBook(b);
          }
        })
        .catch(() => {});
    // Bonds: timestamp for the stale pill + the Stable Bonds ACTIVE book value.
    // Kite-demat bonds live in the snapshot (role debt-equiv); the Stable
    // platform actives exist only in bonds.json, so without this the Overview
    // bonds slice + net worth understate by the active book (~₹80K).
    // Valued at invested capital (no live mark exists for private SDIs),
    // matching the Bonds tab headline. Never gates the paint.
    const loadBonds = () =>
      fetch("/api/bonds")
        .then((r) => r.json())
        .then((r) => {
          ...
          const totals = r?.data?.totals as
            | {
                activeInvestedINR?: number;
                activeValueINR?: number;
                stableSideValueINR?: number;
                activeCount?: number;
              }
            | undefined;
          if (totals && typeof totals.activeInvestedINR === "number") {
            const sb = {
              // Everything held outside the Kite demat: active Stable bonds at
              // cost+accrued PLUS any redemption receivable (units already
              // extinguished, principal still owed). Kite-demat bonds arrive
              // separately via the snapshot, so they must not be added here.
              value:
                totals.stableSideValueINR ??
                totals.activeValueINR ??
                totals.activeInvestedINR,
              count: totals.activeCount ?? 0,
            };
            overviewCache.stableBonds = sb;
            setStableBonds(sb);
          }
        })
        .catch(() => {});
    ...
    Promise.allSettled([loadSnapshot(), loadMf(), loadUs(), loadBook()]).then(() => {
  ...
  const stats = useMemo(() => {
    ...
    // Canonical asset-class split — role-first classification + bucket sums
    // all live in lib/book.ts (single source of truth shared with the Bonds
    // tab, /api/sync, and /api/score).
    const split = splitSnapshotByAsset(holdings);
    const bondHoldings = split.bonds;
    const preciousHoldings = split.metals;
    const inEquityHoldings = split.inEquity;
    ...
    const inEquityValue = book?.byAssetClass["in-equity"].value ?? split.inEquityValue;
    // Whole bond book via lib/book: Kite-demat rows (live) + Stable actives
    // (invested basis). Bonds stay excluded from the stale-nudge rotation.
    const stableBondsValue = stableBonds?.value ?? 0;
    const bondValue = book?.byAssetClass.bonds.value ?? bondBookINR(split.bondsValue, stableBondsValue);
    const preciousValue = book?.byAssetClass.metals.value ?? split.metalsValue;
    ...
    const risk = computeRisk(
      ...
      snapshot.totalValue,
      snapshot.peakValue,
      // Full-book current value so the drawdown compares like-for-like against
      // the full-book peakValue (was IN-only → fake ~64% drawdown).
      snapshot.totalPortfolioValue ?? snapshot.totalValue
    );

    // Combined book: IN stocks + MFs + US stocks + cash
    const cash = book?.byAssetClass.cash.value ?? snapshot.cash ?? 0;
    const mfValue =
      book?.byAssetClass.mf.value ??
    ...
    const usValue = book?.byAssetClass["us-equity"].value ?? usStocks?.totals.currentINR ?? 0;
    const usInvested = usStocks?.totals.investedINR ?? 0;
    const usPnL = usStocks?.totals.pnlINR ?? 0;
    const usHasCost = usInvested > 0;

    const netWorth =
      book?.totals.netWorth ??
      netWorthINR({
        snapshotTotalINR: directHoldingsValue,
        stableBondsINR: stableBondsValue,
        cashINR: cash,
        mfINR: mfValue,
        usINR: usValue,
      });
    ...
    const totalPnL =
      book?.totals.pnlAbs ??
      directPnL + (mfHasCost ? mfPnL : 0) + (usHasCost ? usPnL : 0);
    ...
    const assetItems: AssetAllocationItem[] = [
      ...
      },
      {
        ...
        countLabel: `${bondHoldings.length + (stableBonds?.count ?? 0)} ${
          bondHoldings.length + (stableBonds?.count ?? 0) === 1 ? "bond" : "bonds"
        }`,
        extraLabel: "Stable + Kite demat",
    ...
    return {
      ...
      bondValue,
      bondCount: bondHoldings.length + (stableBonds?.count ?? 0),
      preciousValue,
  ...
  }, [snapshot, mfSummary, usStocks, stableBonds, book, goToTab]);
  ...
  return (
    <>
      ...
      {/* Portfolio score — book discipline vs doctrine; self-fetches /api/score.
          Tapping it opens the Portfolio score tab with the full breakdown. */}
      <PortfolioScoreCard linkToDetail />
    ...
    <section>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="surface rounded-lg p-5">
          ...
          <div className="type-caption text-tertiary mt-1.5 mono">
            equity {risk.equityPct.toFixed(0)}% · metals {risk.metalsPct.toFixed(0)}% · bond {risk.bondsPct.toFixed(0)}%
          </div>
        </div>
      </div>
    </section>
  ...
  const inEquityHoldings = useMemo(() => {
    ...
    return snapshot.holdings.filter(
      (h) =>
        ...
        // SDI/NCD rows live in the snapshot (Kite demat) but belong on the
        // Bonds tab — tickerMeta doesn't know these tickers, so the asset
        // check above misses them; the role field is authoritative.
        h.role !== "debt-equiv" &&
        (h.value ?? 0) > 0 &&
        (h.qty ?? 0) > 0
    );
...
type BondPosition = {
  ...
  issuer: string;
  maturityDate: string;
  // "redeeming": units extinguished from the demat ahead of maturity, principal
  // still owed — sits between transferred and matured.
  status: "active" | "matured" | "transferred" | "redeeming";
  kiteTicker?: string;
  currentINR?: number;
  /** Principal still owed on a "redeeming" bond whose units already left the
   *  demat — counted in the book so net worth doesn't drop before payout. */
  redemptionReceivableINR?: number;
  units: number;
  facePerUnit: number;
...
type BondsData = {
  ...
  totals: {
    investedINR: number;
    activeInvestedINR: number;
    /** Cost + accrued interest, from the Stable Bonds holdings report. Absent
     *  until a valuation has been pulled — callers fall back to cost. */
    activeValueINR?: number;
...
function BondsTab() {
  ...
  // Bonds moved to the Kite demat: descriptive fields from bonds.json, live
  // value joined from the snapshot by the route. Tagged role="debt-equiv", so
  // they land in the snapshot's bondsValue (NOT equityValue) and the IN-equity
  // tab + Overview exclude them — shown here as part of the bond book, no
  // double counting.
  const kiteDemat = useMemo(
    () => (data?.positions ?? []).filter((p) => p.status === "transferred"),
    [data]
  );
  // "redeeming" = units already extinguished from the demat ahead of maturity,
  // principal still owed. Not active, not matured — but excluding it entirely
  // understated the book by the receivable (~₹10K on Indel).
  const redeeming = useMemo(
    () => (data?.positions ?? []).filter((p) => p.status === "redeeming"),
    [data]
  );
  ...
  const stats = useMemo(() => {
    ...
    // Kite-demat bonds are active bonds too — just held in the Kite demat
    // rather than on Stable Bonds. Roll them into the headline book value +
    // count (at their live snapshot value, matching the "In Kite demat" block
    // below and the Overview bonds slice) so the bond section reflects the
    // whole active book, not just the Stable-platform slice.
    const kiteDematValue = kiteDemat.reduce(
      (s, p) => s + (p.currentINR ?? p.investedINR ?? 0),
      0
    );
    // Both sides at value, not cost: Kite at live LTP, Stable at cost+accrued
    // from the holdings report. Mixing the two bases made this disagree with
    // the Overview bonds slice.
    const redeemingValue = redeeming.reduce(
      (s, p) => s + (p.redemptionReceivableINR ?? p.currentINR ?? p.investedINR ?? 0),
      0
    );
    const bookValue =
      bondBookINR(kiteDematValue, totals.activeValueINR ?? totals.activeInvestedINR) +
      redeemingValue;
    const bookCount = totals.activeCount + kiteDemat.length + redeeming.length;
    return {
      totals, weightedYield, next, kiteDematValue, bookValue, bookCount, redeemingValue,
    };
  }, [data, active, kiteDemat, redeeming]);
      ...
      >
        {stats && (
          <>
            <CompactStat
              label="Active value"
              info="Value of all bonds you still have money in. Stable Bonds positions are valued at cost plus accrued interest (private SDIs have no market quote); coupons already paid to your bank are counted under Interest earned, not here. Kite-demat bonds are at live value. 'Awaiting payout' = units already debited from the demat ahead of maturity, principal still owed — carried at face so net worth doesn't dip before the cash lands. Matured bonds excluded."
              value={`₹${fmtINR(Math.round(stats.bookValue))}`}
              sub={`${stats.totals.activeCount} Stable · ${kiteDemat.length} Kite demat${
                redeeming.length ? ` · ${redeeming.length} awaiting payout` : ""
              }`}
            />
            <CompactStat
      ...
      {kiteDemat.length > 0 && (
        <section>
          <div className="flex items-baseline gap-2.5 pb-1">
            <h2 className="text-[15px] font-semibold tracking-[-0.005em] text-tertiary">
              In Kite demat
            </h2>
            <span className="mono-true text-[12.5px] text-tertiary">
              {kiteDemat.length} · ₹{fmtINR(Math.round(kiteDemat.reduce((s, p) => s + (p.currentINR ?? p.investedINR), 0)))} — counted in Bonds totals
            </span>
          </div>
            ...
            {kiteDemat.map((p, i) => (
              <li key={p.isin} style={{ ["--idx" as string]: i }}>
                <BondRow
                  ...
                  onOpen={
                    p.kiteTicker
                      ? () => openPerTicker({ ticker: p.kiteTicker!, market: "BONDS" })
                      : undefined
                  }
                />
              </li>
...
function BondRow({
  ...
  // Defensive: a hand-edited bonds.json row without `issuer` must not crash the
  // whole tab (it did on 2026-07-03 — undefined.split in the avatar initials).
  const couponInitials = (p.issuer ?? p.name ?? "?")
    .split(/\s+/)
    .map((w) => w[0] ?? "")
    .join("")
    .toUpperCase()
...
/**
 * A value that changes with one short cross-fade rather than animating its
 * digits.
 *
 * This started as a per-digit odometer, which was the wrong instinct: the
 * readout updates on every hover, so a widget used dozens of times a day was
 * spinning ten digit columns for what is really a small label change. The
 * width is reserved for the longest value the slot can hold, so a value
 * gaining or losing a place doesn't shift anything — the only motion left is
 * the fade itself.
 */
  ...
  return (
    <span
      ...
      style={{
        ...
        minWidth: minChars ? `${minChars}ch` : undefined,
      }}
    >
      <span key={text} className="value-swap">
        {text}
      </span>
    </span>
  );
...
function AssetSplitStrip({
  ...
  // Memoized so the identity is stable across hovers — recharts restarts its
  // sweep animation whenever `data` changes identity, which made every hover
  // re-draw the chart.
  const data = useMemo(
    () =>
      allocItems.map((it) => ({
    ...
    [allocItems, allocTotal]
  );
  const [activeKey, setActiveKey] = useState<string | null>(null);
  // Leaving the widget doesn't drop the selection immediately — it lingers,
  // so crossing a gap between the ring and the list (or between two rows)
  // doesn't flash the whole thing off and back on. All three helpers are
  // referentially stable, so passing selectKey into the chart memo is safe.
  ...
  const scheduleClear = useCallback(() => {
    cancelClear();
    // Just long enough to bridge the 1px seam between rows (and the ring-to-
    // list gap) without the pointer's own travel dropping the selection.
    // Anything longer stops feeling like grace and starts feeling like lag.
    leaveTimer.current = window.setTimeout(() => setActiveKey(null), 140);
  }, [cancelClear]);
  ...
  const activeIdx = data.findIndex((d) => d.key === activeKey);
  // Built once per data change. Hover must NOT be a dependency: recharts
  // discards and re-creates its <path> nodes on re-render, and a freshly
  // mounted node paints at its final state, so the transition never runs.
  // Keeping this element referentially stable lets React skip the subtree
  // entirely and leaves the slices to pure CSS.
  const chart = useMemo(
    () => (
      <ResponsiveContainer width="100%" height="100%">
        <PieChart>
          {/* Visible ring. Purely decorative — `alloc-slices` is
              pointer-events:none in CSS so it never hit-tests. */}
          <Pie
            className="alloc-slices"
            data={data}
            dataKey="value"
            nameKey="name"
            cx="50%"
            ...
            cornerRadius={4}
            stroke="none"
            // recharts' own pie animation silently renders NO sectors at this
            // radius — the identical Pie without it draws fine. The mount
            // reveal is done in CSS on the wrapper instead (`alloc-pie-in`).
          ...
          {/* Invisible hit layer: a wider band with no padding gaps, so the
              pointer target is far more forgiving than the 26px visible ring
              and there are no dead zones between slices. It never scales, so
              it can't move out from under the cursor and cause flicker. */}
          <Pie
            className="alloc-hit"
            data={data}
            dataKey="value"
            ...
            stroke="none"
            isAnimationActive={false}
            onMouseEnter={(_, i) => selectKey(data[i]?.key ?? null)}
            onClick={(_, i) => data[i]?.onClick?.()}
            style={{ cursor: "pointer", outline: "none" }}
          >
            {data.map((d) => (
              <Cell key={d.key} fill="transparent" stroke="none" />
            ))}
          </Pie>
        </PieChart>
      </ResponsiveContainer>
  ...
  // One highlight element travels between rows rather than each row toggling
  // its own background — a background swap can only cross-fade, it can't move.
  const listRef = useRef<HTMLUListElement>(null);
  const [hl, setHl] = useState<{
    top: number;
    height: number;
    /** Distance that parks the block just past the list's bottom edge. With no
     *  fade to hide it, it has to travel fully out of view. */
    exitY: number;
  } | null>(null);
  const [hlShown, setHlShown] = useState(false);
  // Vertical travel is only animated when the highlight was ALREADY visible
  // (moving row to row). On first appearance it snaps to the right row with no
  // Y transition, so the entrance is a clean slide in from the edge rather
  // than a diagonal drift down from row one.
  ...
  useEffect(() => {
    ...
    if (!root || !activeKey) {
      // Keep the last geometry so it exits from where it stands.
      wasShownRef.current = false;
      setHlShown(false);
      return;
    }
    const row = root.querySelector<HTMLElement>(
      `[data-alloc-row="${activeKey}"]`
    );
    if (!row) return;
    // Always in and out via the bottom edge.
    const exitY = Math.max(0, root.clientHeight - row.offsetTop);
    setHl({ top: row.offsetTop, height: row.offsetHeight, exitY });
    const hadSelection = wasShownRef.current;
    setAnimateY(hadSelection);
    wasShownRef.current = true;
    if (hadSelection) {
      setHlShown(true);
      return;
    }
    // First selection: paint once parked below the list, then animate up on a
    // later frame. Flipping both in one commit would give React nothing to
    // transition from, and with the fade gone there'd be no entrance at all.
    setHlShown(false);
    const outer = requestAnimationFrame(() => {
      enterFrameRef.current = requestAnimationFrame(() => setHlShown(true));
    });
    enterFrameRef.current = outer;
  }, [activeKey]);
  // Centre readout targets. These are plain strings — the roll happens in the
  // digit columns via transform, so state only changes when the value does.
  const readoutValue = fmtINR(active ? active.value : allocTotal);
  const readoutPct =
    active && allocTotal > 0
      ? `${((active.value / allocTotal) * 100).toFixed(1)}%`
      : null;
  // Widest value this slot can ever show — the total is the longest, but check
  // the items too so a fresh book with one huge holding still reserves enough.
  const readoutWidth = useMemo(
    () =>
      Math.max(
        fmtINR(allocTotal).length,
        ...allocItems.map((it) => fmtINR(it.value).length)
      ),
    [allocItems, allocTotal]
  );
  ...
  return (
    <section className="surface rounded-lg overflow-hidden">
      ...
      >
        ...
        >
          {/* `data-active-idx` drives the slice lift/dim from CSS. The chart
              element itself is memoized and never re-renders on hover —
              recharts rebuilds its <path> nodes on every render, and a fresh
              node paints at its end state, so any CSS transition was being
              skipped. Only this wrapper re-renders now. */}
          <div
            className="alloc-pie relative"
            style={{ width: 200, height: 200 }}
            data-active-idx={activeIdx >= 0 ? activeIdx + 1 : undefined}
            // Clearing lives here, not on the Pie: this box never scales, so
            // it's a stable boundary for "the pointer actually left the ring".
            onMouseEnter={cancelClear}
            onMouseLeave={scheduleClear}
          >
            {chart}
            {/* The wrapper is deliberately NOT keyed — remounting it would
                restart the roll. Only the two text labels are keyed, so they
                cross-fade while the digits keep spinning underneath. */}
            {/* No optical nudge, and that is measured rather than assumed.
                These lines are all lining figures and caps with no descenders,
                so comparing line-box rects suggests the block sits low — but
                line boxes include leading. Measuring real glyph ink (canvas
                actualBoundingBox against a baseline probe) puts the ink centre
                within 0.2px of the ring's centre with no transform at all. */}
            <div className="absolute inset-0 flex flex-col items-center justify-center text-center pointer-events-none">
              <span
                key={active?.key ?? "__all"}
                className="alloc-readout text-[9.5px] uppercase tracking-[0.08em] text-tertiary leading-none"
              >
                {active ? active.label : "Allocated"}
              </span>
              <span className="mono-true font-semibold text-primary text-[15px] tracking-tight leading-none mt-1.5 tabular-nums">
                <ReadoutValue
                  text={`₹${readoutValue}`}
                  minChars={readoutWidth + 1}
                />
              </span>
              <span className="mono-true text-[10px] text-tertiary mt-1.5 leading-none tabular-nums">
                {readoutPct ? (
                  <ReadoutValue text={readoutPct} minChars={5} />
                ) : (
                  <span key="__count" className="alloc-readout inline-block">
                    {`${allocItems.length} ${
        ...
        {/* overflow-hidden so the highlight visibly exits past the edge
            instead of hanging off the panel while it fades. */}
        <ul
          ref={listRef}
          className="relative flex flex-col pt-2 overflow-hidden"
        >
          {/* Travels to the hovered row. Sits behind the rows, so the text
              stays put while only the highlight moves. */}
          {/* Outer layer owns vertical position, inner owns the slide + fade.
              Splitting them lets the entrance animate X only while Y is
              already correct, and lets row-to-row travel animate Y only. */}
          <span
            aria-hidden="true"
            className="alloc-legend-highlight pointer-events-none absolute left-0 right-0 top-0"
            style={{
              height: hl?.height ?? 0,
              transform: `translate3d(0, ${hl?.top ?? 0}px, 0)`,
              transition: animateY
                ? "transform 420ms cubic-bezier(0.22, 1, 0.36, 1), height 420ms cubic-bezier(0.22, 1, 0.36, 1)"
                : "none",
            }}
          >
            <span
              ...
              style={{
                background: "var(--bg-subtle)",
                // Pure slide, no fade — it enters from below the list and
                // leaves the same way, matching the row-to-row travel so the
                // widget speaks one direction of motion throughout.
                transform: `translate3d(0, ${hlShown ? 0 : hl?.exitY ?? 0}px, 0)`,
                transition: hlShown
          ...
          {allocItems.map((item, i) => {
            ...
            return (
              <li
                key={item.key}
                data-alloc-row={item.key}
                onMouseEnter={() => selectKey(item.key)}
                onMouseLeave={scheduleClear}
                onClick={item.onClick}
                className={`relative px-5 md:px-6 py-3.5 flex items-center gap-3 ${
                  item.onClick ? "cursor-pointer" : ""
                }`}
                style={{
                  borderTop:
                    i > 0 ? "1px solid var(--border)" : undefined,
                  // Nudges toward the pie on hover so the row and its slice
                  // read as one gesture rather than two separate highlights.
...
type Task = {
  ...
  anchor?: TaskAnchor;
  // Fallback "+N" score points for tasks the doctrine levers can't match by
  // ticker — a fresh-capital deploy earns its share of the deploy-floor lever
  // even though that lever carries no ticker. Ticker-matched levers still win.
  scoreImpact?: number;
};
...
type Decision = {
  ...
  reviewAt?: string;
  note?: string;
  // Rule 10 / rule 5 stamps (v2 rulebook, 2026-09-16)
  rule?: string;
  thesis?: string;
  exitIf?: string;
  trackingNote?: string;
  ...
  return (
    <div className="space-y-7">
      <PageHero
        ...
        info="What the composite is made of, and the actions that move it. Each dimension shows where the book drifts from your doctrine; tasks carrying a +N badge are the levers that close the gap."      />
      {/* The score breakdown leads — the queue below is how you act on it.
          Header suppressed: the PageHero above already names the page. */}
      <PortfolioScoreCard hideHeader hideLevers />
      <section>
        <TasksSection />
      </section>
    </div>
                ...
                >
                  {rows.map((t, i) => (
                    <TaskListRow
                      ...
                      t={t}
                      idx={i}
                      scoreGain={(t.ticker ? scoreByTicker.get(t.ticker.toUpperCase()) : undefined) ?? t.scoreImpact}
...
function DecisionsSection() {
  ...
  const liveGood = tracked.filter(
    (i) => i.outcome === "winning" || i.outcome === "saved"
  ).length;
  // Recorded verdicts lead (Phase 3.2 of /portfolio-check re-scores them every
  // sync); the live price outcome is only the fallback before anything has
  // been verdicted. "Reviews due" = pending verdicts whose review date passed.
  const goodV = filtered.filter((i) => i.verdict === "good").length;
  const badV = filtered.filter((i) => i.verdict === "bad").length;
  const pendingV = filtered.filter((i) => i.verdict === "pending").length;
  const todayIso = new Date().toISOString().slice(0, 10);
  const reviewsDue = filtered.filter(
    (i) => i.verdict === "pending" && !!i.reviewAt && i.reviewAt <= todayIso
  ).length;
  // Hit rate counts EVERY decision that can be read: a recorded verdict wins,
  // otherwise the live price outcome stands in (winning/saved = good,
  // losing/missed = bad). Flat / exited / unpriced rows are excluded. A
  // verdict-only rate would read 100% with zero "bad" stamps while a third of
  // the book sits underwater — that is not a hit rate, it is a filing gap.
  const pendingLive = filtered.filter((i) => i.verdict === "pending");
  const liveGoodPending = pendingLive.filter(
    (i) => i.outcome === "winning" || i.outcome === "saved"
  ).length;
  const liveBadPending = pendingLive.filter(
    (i) => i.outcome === "losing" || i.outcome === "missed"
  ).length;
  const good = goodV + liveGoodPending;
  const bad = badV + liveBadPending;
        ...
        {statsOpen && (
          <div className="surface rounded-lg overflow-hidden">
            <div className="grid grid-cols-2 md:grid-cols-4 -m-px">
              ...
              <CompactStat
                label={`Verdicts · ${pendingV} pending${reviewsDue > 0 ? ` · ${reviewsDue} due` : ""}`}
                value={`${goodV} good · ${badV} bad`}
                accent={badV > goodV ? "neg" : goodV > 0 ? "pos" : undefined}
              />
              <CompactStat
                label="Live read · pending"
                value={`${liveGoodPending} up · ${liveBadPending} down`}
                accent={liveBadPending > liveGoodPending ? "neg" : liveGoodPending > 0 ? "pos" : undefined}
              />
              <CompactStat
                label={`Hit rate · ${total} read`}
    ...
    >
      <div className="flex items-start gap-3.5">
        ...
        <div className="flex-1 min-w-0">
          ...
          {d.exitIf && (
            <p className="text-[11.5px] text-tertiary mt-1.5 leading-relaxed max-w-[68ch]">
              <span className="font-medium text-secondary">Exit if</span> {d.exitIf}
            </p>
          )}
          {(outcomeBadge || verdictBadge || sellHint || d.rule) && (
            <div className="flex items-center gap-2 mt-2.5 flex-wrap">
              {outcomeBadge}
              {verdictBadge}
              {d.rule && <Tag label={`rule ${d.rule}`} tone="neutral" />}
              {sellHint && (
