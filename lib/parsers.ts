...
export type MFSummary = {
  /** True only when live mfapi NAVs were actually overlaid (set by enrichMF). */
  navsLive?: boolean;
  asOf?: string;
...
export function parseMutualFunds(md: string): MFSummary {
  ...
  if (snapBlock) {
    ...
    const get = (name: string) => {
      // Tolerate a trailing qualifier before the colon, e.g.
      // "Invested (new cost basis):" or "Net P&L on current cost basis:" —
      // the field name is the anchor; anything up to the first colon is ignored.
...
function parseMFBlock(block: string): MFEntry | null {
  ...
  const get = (name: string) => {
    // Tolerate a trailing qualifier before the colon, e.g.
    // "Invested (new cost basis):" or "P&L on new basis:" — the field name is
    // the anchor; anything up to the first colon is ignored.
    const re = new RegExp(`-\\s*${name}[^:\\n]*:\\s*(.+)`, "i");
    const m = block.match(re);
    return m ? m[1].trim() : undefined;
  };
  ...
  // Accept "Value:" or "Current Value:" (some switched-fund blocks use the latter).
  const value = parseNumber(get("Value") ?? get("Current Value")) ?? 0;
